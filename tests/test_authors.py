...
def test_get_authors(client, sample_data):
    ...
    assert response_data['number_of_record'] == 5
...
def test_get_authors_with_params(client, sample_data):
    ...
    assert response.headers['Content-Type'] == 'application/json'
    assert response_data['success'] is True
    assert response_data['number_of_record'] == 2
    assert response_data['pagination'] == {
            'total_pages': 5,
            'total_records': 10,
...
def test_create_author(client,author):
...
def test_create_author_invalid_data(client, data, missing_field):
...
def test_create_author_invalid_content_type(client, author):
    response = client.post('/api/v1/authors',
                           data=author)
    response_data = response.get_json()
